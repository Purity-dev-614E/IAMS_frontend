...
class EndOfAttachmentReportService {
  ...
  async downloadReport(id) {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000/api"}${API_ROUTES.endOfAttachmentReports.download(id)}`, {
        method: 'GET',
        headers: {
  ...
  async staffDownloadReport(id) {
    try {
      const token = localStorage.getItem('iams_token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000/api"}${API_ROUTES.endOfAttachmentReports.staffDownload(id)}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        }
